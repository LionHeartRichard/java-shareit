...
public interface BookingRepository extends JpaRepository<Booking, Long> {
	...
	@Query("SELECT b FROM Booking b " + "WHERE b.item.id = :itemId " + "  AND b.end < :currentTime "
			+ "ORDER BY b.start DESC")
	List<Booking> findLastBooking(@Param("itemId") final Long itemId, @Param("currentTime") final Long currentTime,
			Pageable pageable);

	@Query("SELECT b FROM Booking b " + "WHERE b.item.id = :itemId " + "  AND b.start > :currentTime "
			+ "ORDER BY b.start ASC")
	List<Booking> findNextBooking(@Param("itemId") final Long itemId, @Param("currentTime") final Long currentTime,
			Pageable pageable);
